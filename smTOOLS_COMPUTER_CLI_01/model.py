"""smTOOLS_COMPUTER_CLI_01: a tiny byte-level GPT that turns a request into a command line.

    ubuntu: what's using port 8080  <SEP>  sudo ss -ltnp 'sport = :8080'|read  <END>

The platform and the request come first as plain bytes, then the model writes the command and how risky it
thinks it is. Only what it writes is scored. An empty answer means the request isn't a terminal job.
"""
